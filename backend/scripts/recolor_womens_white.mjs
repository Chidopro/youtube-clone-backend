/**
 * Recolor Women's Tools mockups to white flats and punch out the studio grey
 * so Product Preview can tint them to the cart color.
 * Keeps the mint/pink print-area guide. Does not use Printful on-model photos.
 * Shop browse images are not rewritten.
 */
...
const JOBS = [
  { src: 'womenshirtpreview.png', dest: 'womenshirtpreview2.png' },
  { src: 'womenshdshirtpreview.png', dest: 'womenshdshirtpreview2.png' },
  { src: 'womensribbedneckpreview.png', dest: 'womensribbedneckpreview2.png' },
  { src: 'womensmicroribtanktoppreview.png', dest: 'womensmicroribtanktoppreview2.png' },
  { src: 'womenstankpreview.png', dest: 'womenstankpreview2.png' },
  { src: 'womenscroptoppreview.png', dest: 'womenscroptoppreview2.png' },
  { src: 'womensunisexpulloverhoodiepreview.png', dest: 'womensunisexpulloverhoodiepreview2.png' },
  { src: 'womenscroppedhoodiepreview.png', dest: 'womenscroppedhoodiepreview2.png' },
];
...
function densestRun(hits, thresh) {
  let bestS = 0;
  let bestE = -1;
  let bestLen = 0;
  let s = -1;
  for (let i = 0; i <= hits.length; i += 1) {
    const on = i < hits.length && hits[i] >= thresh;
    if (on) {
      if (s < 0) s = i;
    } else if (s >= 0) {
      const len = i - s;
  ...
  return bestE >= bestS ? [bestS, bestE] : null;
}
...
function findPrintRect(data, w, h) {
  ...
  const rowRun = densestRun(rowHits, Math.max(8, Math.round(w * 0.12)));
  const colRun = densestRun(colHits, Math.max(8, Math.round(h * 0.08)));
  ...
  const cap = Math.min(h - 1, minY + Math.round(boxW * 1.45));
  const denseNeed = Math.max(8, Math.round(boxW * 0.35));
  let y2 = maxY;
  for (let y = maxY; y <= cap; y += 1) {
    let n = 0;
    for (let x = minX; x <= maxX; x += 1) {
      const i = (y * w + x) * 4;
      if (isPrintBoxColor(data[i], data[i + 1], data[i + 2])) n += 1;
    }
    if (n >= denseNeed) y2 = y;
  }
  return {
...
function processBuffer(buf) {
  ...
  return {
    ...
    p5: Math.round(p5),
    p95: Math.round(p95),
    rect: rect ? `${rect.x1},${rect.y1}-${rect.x2},${rect.y2}` : 'none',
  };
}

for (const job of JOBS) {
  const src = path.join(imagesDir, job.src);
  const dest = path.join(imagesDir, job.dest);
  const result = processBuffer(fs.readFileSync(src));
