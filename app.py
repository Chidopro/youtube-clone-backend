...
@app.route("/api/products/<product_name>/variants", methods=["GET"])
def get_product_variants(product_name):
    """Get all variants (color/size combinations) for a specific product"""
    try:
        ...
        for color in colors:
            for size in sizes:
                ...
                size_adjustment = size_pricing.get(size, 0)
                variant_price = base_price + size_adjustment
    ...
    try:
        ...
        for product in PRODUCTS:
            products_summary.append({
                ...
                "colors": product.get('options', {}).get('color', []),
                "sizes": product.get('options', {}).get('size', []),
                "total_variants": len(product.get('options', {}).get('color', [])) * len(product.get('options', {}).get('size', []))
            })
