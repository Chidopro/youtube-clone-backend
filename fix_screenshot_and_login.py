#!/usr/bin/env python3
"""
Fix Screenshot Capture and Login Issues
"""
...
def create_screenshot_capture_module():
    """Create the missing screenshot_capture module"""
    
    screenshot_capture_code = '''#!/usr/bin/env python3
"""
...
'''
    ...
    # Write the screenshot_capture module
    with open('screenshot_capture.py', 'w', encoding='utf-8') as f:
        f.write(screenshot_capture_code)
    
    print("✅ Created screenshot_capture.py module")

def fix_login_timeout():
    """Add timeout handling to login requests"""
    
    # Read the current backend app.py
    with open('backend/app.py', 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Add timeout to requests
    if 'requests.get(' in content and 'timeout=' not in content:
        content = content.replace('requests.get(', 'requests.get(timeout=30, ')
        content = content.replace('requests.post(', 'requests.post(timeout=30, ')
    
    # Write back
    with open('backend/app.py', 'w', encoding='utf-8') as f:
        f.write(content)
    
    print("✅ Added timeout handling to backend requests")

def main():
    print("🔧 Fixing screenshot capture and login issues...")
    
    # Create screenshot capture module
    create_screenshot_capture_module()
    ...
    print("   2. Test the screenshot capture functionality")
